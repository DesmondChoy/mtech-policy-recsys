...
This script generates a Markdown summary report from existing ground truth evaluation JSON data
without re-running the evaluations. It's useful when you've already generated the individual 
customer evaluations and just want to update the summary.
...
def generate_markdown_summary(json_path, output_path=None):
    ...
    if not output_path:
        output_path = os.path.join(os.path.dirname(json_path), "coverage_evaluation_summary.md")
...
def main():
    parser = argparse.ArgumentParser(description='Generate Markdown summary from ground truth evaluation JSON')
