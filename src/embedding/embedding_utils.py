...
class EmbeddingMatcher:
    ...
    def find_best_match(self, query_key, threshold=0.7, use_fuzzy=True):
        """
    ...
    def get_value(self, query_key, threshold=0.7, use_fuzzy=True):
        """
        ...
        """
        best_match, score, level = self.find_best_match(query_key, threshold, use_fuzzy)
        if best_match:
            return self.ground_truth[best_match], (best_match, score, level)
        else:
            return [], (None, 0.0, None)
            
    def get_values_batch(self, queries, threshold=0.7, use_fuzzy=True):
        """
        ...
        for query in queries:
            best_match, score, level = self.find_best_match(query, threshold, use_fuzzy)
            if best_match:
                results[query] = {
